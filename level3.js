...
function sodukuGame() {
  ...
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 9; j++) {
      ...
      document.getElementById("game3").appendChild(box);
      ...
      if (
        ...
        box.style.backgroundColor = "rgb(247, 158, 158)"; //darker pink than the board color
      }
    }
  }
}
...
function sodukuNums() {
  ...
  for (let i = 1; i < 10; i++) {
    ...
    number.setAttribute("class", "num3");
    ...
    number.addEventListener("click", function select() {
      if (numSelected != null) {
        numSelected.classList.remove("number-selected3"); //if the corrent value dosent matches to 'this'
      }
      numSelected = this; //captures the value of ths button the player preesed
      numSelected.classList.add("number-selected3");
...
function closePopup() {
  ...
  if (popupF.classList.remove("open-popupF")) {
    location.href = "gameL3.html"; // if the player faild the lavel, the game restart it self.
  }
}
