...
function sodukuGame() {
  ...
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 9; j++) {
      ...
      document.getElementById("game1").appendChild(box);

      //
      // //reviles the right number in the tile
      ...
      if (
        ...
        box.style.backgroundColor = "rgb(158, 247, 184)"; //darker green than the board color
      }
    }
  }
}
...
let counter = 0;
while (counter < 25) {
  board[i][j] = ""; //hide to content of the cell
...
function sodukuNums() {
  ...
  for (let i = 1; i < 10; i++) {
    ...
    number.addEventListener("click", function select() {
      if (numSelected != null) {
        numSelected.classList.remove("number-selected1"); //if the corrent value dosent matches to 'this'
      }
      numSelected = this; //captures the value of ths button the player preesed
      numSelected.classList.add("number-selected1");
...
//this function chacks if all the tiels are filled
function finishGame() {
  let flag = true;
  ...
  //open the popup if the player solved the level and won the game
  if (flag == true) {
...
function closePopup() {
  ...
  if (popupF.classList.remove("open-popupF")) {
    location.href = "gameLl.html"; // if the player faild the lavel, the game restart it self.
  }
}
